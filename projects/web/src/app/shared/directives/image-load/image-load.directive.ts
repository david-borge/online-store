...
export class ImageLoadDirective {
    ...
    switch (this.currentURL) {
      ...
      case '/categories':
        ...
        if ( !this.categoriesPageImagesLoaded ) {
          this.store.dispatch( CategoriesActions.IncrementInOneTheNumberOfImagesInThisPage() );
        }
  ...
  @HostListener('load') onLoad(): void {
    ...
    switch (this.currentURL) {
      ...
      case '/home':
        ...
        if ( !this.homePageImagesLoaded ) {
          this.store.dispatch( HomeActions.IncrementInOneTheNumberOfImagesInThisPageLoaded() );
        }
      ...
      case '/product':
        ...
        // Aumentar el número de imágenes de la página que han sido cargadas, que está guardado en la Store (si las imágenes todavía no se han cargado)
        if ( !this.productPageImagesLoaded ) {
          this.store.dispatch( ProductActions.IncrementInOneTheNumberOfImagesInThisPageLoaded() );
        }
  ...
  @HostListener('error') onError() {
    ...
    console.log('ImageLoadDirective > error al cargar una imagen: ' + this.elementRef.nativeElement.src);
