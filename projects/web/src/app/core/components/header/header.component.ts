...
@Component({
  ...
  styles: [`
    :host {
      --step-bar-percentaje: var(--default-color); // Set a default value for the CSS variable
    }
  `],
})
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.globalReducerObservableSubscription = this.store.select('globalReducerObservable').subscribe(
      (globalReducerData) => {
  ...
  onClickGoBack(): void {
    ...
    if ( currentURL.includes('/checkout/payment-method') || currentURL.includes('/checkout/order-review') ) {
