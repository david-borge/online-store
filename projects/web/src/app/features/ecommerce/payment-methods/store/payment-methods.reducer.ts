...
export const paymentMethodsReducer = createReducer(
    ...
    on(PaymentMethodsActions.GetPaymentMethodsEndSuccess, (state, action) => ({
        // El Reducer devuelve la App State ya alterada por la Action (aka Reduced State).
    ...
    on(PaymentMethodsActions.AddNewCardEndSuccess, (state, action) => {
            ...
            paymentMethods: [
                {
                    ...
                    cardLastFourNumbers: action.newCardPayload.cardNumber.substring(
                        action.newCardPayload.cardNumber.length - 4,
                    ), // Los 4 últimos números
    ...
    on(PaymentMethodsActions.AddNewCardEndFailure, (state, action) => ({
        // El Reducer devuelve la App State ya alterada por la Action (aka Reduced State).
    ...
    // Side Effects asociados: addNewCardSideEffect (toma los datos de la Cardes desde la base de datos mediante un HTTP Request)
    on(PaymentMethodsActions.SaveNewCardToStore, (state, action) => ({
        /* Añadir un valor */
        // El Reducer devuelve la App State ya alterada por la Action (aka Reduced State).
    ...
    on(PaymentMethodsActions.ChangeDefaultPaymentMethodStart, (state, action) => {
        // El Reducer devuelve la App State ya alterada por la Action (aka Reduced State).
        ...
        updatedPaymentMethods[action.paymentMethodArrayIdPayload] = {
            ...updatedPaymentMethods[action.paymentMethodArrayIdPayload],
            isDefault:
                updatedPaymentMethods[action.paymentMethodArrayIdPayload]['isDefault'] == 0 ? 1 : 0, // Cambiar el valor de 0 a 1 o viceversa
        };
